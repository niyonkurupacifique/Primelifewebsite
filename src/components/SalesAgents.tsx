...
import HeroSalesAgent from "./HeroSalesAgent";
...
const SalesAgentComponents = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      socket.on('Sales_Agent_created', onChange)
      socket.on('Sales_Agent_updated', onChange)
      socket.on('Sales_Agent_deleted', onChange)
    } catch (e) {
      console.error('Socket connection failed', e)
    }
    return () => {
      if (socket) {
        socket.off('Sales_Agent_created')
        socket.off('Sales_Agent_updated')
        socket.off('Sales_Agent_deleted')
        socket.disconnect()
      }
    }
  }, [])
  ...
  const headerText = useMemo(() => {
    ...
    return `PRIME LIFE QUALIFIED SALES AGENTS ${quarterLabel(quarterCode)} / ${yearStr}.`
  }, [])
        ...
        <section className="relative py-12 sm:py-16 lg:py-20" data-aos="fade-up">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="mb-8 sm:mb-10">
              <h2 className="text-2xl sm:text-3xl font-semibold text-gray-900">Find a Sales Agent</h2>
...
export default SalesAgentComponents