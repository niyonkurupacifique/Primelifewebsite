...
interface ParentBeneficiariesProps {
  title?: string;
...
const ParentBeneficiaries: React.FC<ParentBeneficiariesProps> = ({ 
  ...
  // Get parent counts from family quotation form
  ...
  // Calculate total required parent beneficiaries
  const totalRequiredParents = directParent + directParentInLaw;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      if (editingIndex !== null && isEditing) {
        ...
        setSuccessMessage('Parent beneficiary information updated successfully!');
        setIsEditing(false);
        ...
        setSuccessMessage('Parent beneficiary information added successfully!');
  ...
  const handleRemoveBeneficiary = (index: number) => {
    ...
    setSuccessMessage('Parent beneficiary removed successfully!');
    setTimeout(() => {
      setSuccessMessage('');
    }, 3000);
  };
  ...
  const handleNext = () => {
    if (beneficiaries.length >= totalRequiredParents) {
      onNext?.();
    } else if (validateForm()) {
      dispatch(addBeneficiary(formData));
      if (beneficiaries.length + 1 >= totalRequiredParents) {
        onNext?.();
      }
    }
  };
  ...
  // Determine relationship options based on parent counts
  const getRelationshipOptions = () => {
    const options = [];
    
    if (directParent > 0) {
      options.push({ value: 'Direct Parent', label: 'Direct Parent' });
    }
    
    if (directParentInLaw > 0) {
      options.push({ value: 'Direct Parent in Law', label: 'Direct Parent in Law' });
    }
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="rounded-lg shadow-sm border border-[#EBEDF0] p-6">
          ...
          <div className="flex items-center">
            ...
            <div>
              <p className="text-blue-800 text-sm font-medium">
                <strong>Parent Count Summary:</strong>
              </p>
              <p className="text-blue-700 text-sm">
                Direct Parents: {directParent} | Direct Parents in Law: {directParentInLaw} | Total Required: {totalRequiredParents}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className={` ${beneficiaries.length===0?'hidden':''} mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg`}>
          <div className="flex items-center">
            ...
            <p className={`  text-yellow-800 text-sm`}>
              <strong>Progress:</strong> {beneficiaries.length} of {totalRequiredParents} parent beneficiaries added
              {beneficiaries.length < totalRequiredParents && ` (${totalRequiredParents - beneficiaries.length} more required)`}
            </p>
          </div>
        </div>
        ...
        {beneficiaries.length > 0 && (
          <div className="mb-6">
            <h3 className="font-medium text-[#003366] mb-3">Added Parent Beneficiaries</h3>
        ...
        {beneficiaries.length >= totalRequiredParents && onNext && (
          <div className={`${beneficiaries.length===0?'hidden':''}   mb-6 p-4 bg-green-50 border border-green-200 rounded-lg`}>
            <div className="flex items-center">
              <svg className="w-5 h-5 text-green-600 mr-2" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
              </svg>
              <p className="text-green-800 text-sm">
                <strong>Ready to continue!</strong> All required parent beneficiaries have been added. You can now proceed to the next step.
              </p>
            </div>
          </div>
        )}

        {/* Form - Only show if more beneficiaries are needed */}
        {beneficiaries.length < totalRequiredParents && (
          <>
            {/* Form Information */}
            <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <p className="text-blue-800 text-sm">
                <strong>Note:</strong> {isEditing ? 'You are currently editing a parent beneficiary. Click "Update Parent Beneficiary" to save your changes.' : `Fill out the form below to add parent beneficiary ${beneficiaries.length + 1} of ${totalRequiredParents}.`}
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex justify-end space-x-4 pt-6 border-t border-gray-200">
                ...
                >
                  {isEditing ? 'Update Parent Beneficiary' : 'Add Parent Beneficiary'}
                </button>
              </div>
            </form>
          </>
        ...
        {/* Continue Button - Only show if all required beneficiaries are added */}
        {beneficiaries.length >= totalRequiredParents && onNext && (
...
export default ParentBeneficiaries;
