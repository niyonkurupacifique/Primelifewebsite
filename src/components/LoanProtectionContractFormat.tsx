...
const LoanProtectionContractFormat = () => {
  const languages = [
  ...
  const illnessFilelanguage = [
   
    { name: 'English', code: 'en' },
    
  ];
  const contractFiles: Record<string, string> = {
    rw: '/Loan_Protection_Kinyarwanda.pdf',
    en: '/Loan_Protection_English.pdf',
    fr: '/Loan_protection__French.pdf',
  };

  const illnessFile: Record<string, string> = {
   
    en: '/List of Critical Illnesses.pdf',
    
  };

  const proposalForms: Array<{ product: string; url: string }> = [];
    ...
    <div className="max-w-6xl mx-auto p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="rounded-lg p-6 border border-slate-300/40 hover:shadow-xl transition-shadow duration-300">
         ...
         {/* Critical Illnes */}
        <div className="rounded-lg p-6 border border-slate-300/40 hover:shadow-xl transition-shadow duration-300">
          <h2 className="text-2xl font-bold text-[#00AFF3] mb-4">Covered Critical IlInesses </h2>
          <ul className="space-y-3">
            {illnessFilelanguage.map((language) => (
              <li key={language.code} className="flex items-center justify-between">
                <span className="text-gray-800">{language.name}</span>
                <div className="flex items-center space-x-2">
                  <a
                    href={illnessFile[language.code]}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="px-3 py-1.5 bg-white border-2 border-yellow-400 text-gray-700 rounded-md text-sm font-semibold hover:bg-yellow-50 transition-colors duration-200"
                  >
...
export default LoanProtectionContractFormat;